...
def Multiple_ref_clustering_streaming_parameters(windows_updates, vectors, theta, window_num, para_theta, ht1_size, ht2_size, n_c):
    clusters = []
    angle_degrees = torch.tensor(theta)  # 角度 5°
    threshold = torch.deg2rad(angle_degrees)  # 转换为弧度
    vector_objects = {}  # 对象
    ...
    time_each_window = []
    time_full_insert = []
    hash_table_1_Ref = {i: [] for i in range(ht1_size)}
    hash_table_2_Ref = {i: [] for i in range(ht2_size)}
    # base_R1 = None
    base_R1 = Reference(torch.randn(vectors[0].shape[0]))
    proj_R = None
    ...
    for window_index, updates in windows_updates.items():
        ...
        # 再添加
        vector_objects, clusters, vector_to_cluster, time_each_insert, hash_table_1_Ref, base_R1, hash_table_2_Ref, proj_R \
            = Multiple_ref_clustering_incremental_parameters(
            vector_objects,
            updated_vectors,
            clusters,
            threshold, vector_to_cluster, hash_table_1_Ref, base_R1, hash_table_2_Ref, proj_R, ht1_size, ht2_size, n_c)
        t2 = time.time()
        print(t2 - t1)
        time_each_window.append(t2 - t1)
        time_full_insert += time_each_insert
...
def Multiple_ref_clustering_incremental_parameters(vector_objects, updated_vectors, clusters, threshold, vector_to_cluster,
                                        hash_table_1_Ref, base_R1, hash_table_2_Ref, proj_R, ht1_size, ht2_size, n_c):
    time_each_insert = []
    for row, vector_data in updated_vectors.items():
        vector_i = Vector(vector_data)
        vector_objects[row] = vector_i
        ...
        if len(clusters) != 0 and base_R1 is not None:
            # 检查当前vector最近的cluster
            bucket1 = check_hash1_bucket(hash_table_1_Ref, vector_i, base_R1, ht1_size, threshold)
            near_ref, near_slot, min_angle_R1 = find_nearest_reference(bucket1, hash_table_1_Ref, vector_i)
            cluster_to_insert_id = near_ref[0]
            cluster_to_insert = clusters[cluster_to_insert_id]
        # for i in range(len(clusters)):
        ...
        if INSERT:  # 能插入
            # print("YES, insert to existing cluster")
            cluster_to_insert.ht1_size = ht1_size
            cluster_to_insert.ht2_size = ht2_size
            cluster_to_insert.add_vector(vector_i)
            cluster_to_insert.update_old_references(vector_i)
            max_angle_vector = cluster_to_insert.search_max_angle(vector_i)  # search时间太长
            cluster_to_insert.add_new_reference(vector_i, max_angle_vector, n_c)  # 是否能成为新的reference,已经有旧的ref了
            vector_to_cluster[row] = cluster_to_insert_id
        else:  # 不能插入，单独开一个新的聚类插入
            new_cluster = Cluster(threshold, ht1_size, ht2_size)  # 新的Cluster类
            new_cluster.add_vector(vector_i)
            new_cluster.update_old_references(vector_i)
            clusters.append(new_cluster)
            vector_to_cluster[row] = len(clusters) - 1
...
def Multiple_ref_clustering_streaming_Cluster(windows_updates, vectors, theta, window_num, para_theta):
    clusters = []
    angle_degrees = torch.tensor(theta)  # 角度 5°
    threshold = torch.deg2rad(angle_degrees)  # 转换为弧度
    vector_objects = {}  # 对象
